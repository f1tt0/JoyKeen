...
class GameBase(BaseModel):
    ...
    rules_config: Dict[str, Any] = Field(default_factory=dict, alias="rulesConfig")
...
class GameUpdate(BaseModel):
    ...
    description: Optional[str] = None
    rules_config: Optional[Dict[str, Any]] = Field(default=None, alias="rulesConfig")
...
class GameService:
    ...
    def __new__(cls):
        if cls._instance is None:
            ...
            cls._instance.games = [] # In-memory storage для об'єктів GameResponse
        return cls._instance

    def create_game(self, game_data: GameCreate) -> GameResponse:
        # Перетворюємо вхідні дані на об'єкт відповіді (генеруються ID та дата)
